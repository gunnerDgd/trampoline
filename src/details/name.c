...
bool_t 
    name_new_from_str
        (name* par, str* par_str)                                       {
            ...
            if (tok_pos == -1)                         {
                len = str_len(par_str)                 ; if (len >= 64) return false_t;
                str_push_back_cstr(&par->name, &len, 1);
                str_push_back     (&par->name, par_str);
...
bool_t
    name_new_from_pkt
        (name* par, struct pkt* par_pkt, u8_t* par_ptr) {
            u8_t  len;
            u16_t off;
